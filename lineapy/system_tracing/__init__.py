...
1. Use the `record_function_calls.py` context manager to record a number of function calls that happen during it,
   limited to some code. This uses `sys.settrace` to trace every bytecode execution and `_op_stack.py` to look at
...
2. Use `function_calls_to_side_effects.py` to translate the sequence of calls that were recorded into the side effects
   produced on the nodes, by providing it with the initial values that were nodes.
