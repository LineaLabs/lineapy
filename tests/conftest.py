...
def assertion_helper():
    """
    If you plan to use pytest to validate results beyond just snapshots,
    most of our tests either need to check whether a certain variable was
    loaded or has the right value.

    If you have a list of var names, what you want to validate and
    their expected values, simply passing that list to this fixture will
    let you run through all those assertions.

    returns: function that takes the actual response generated by the code and
    a list of expected results.
    """

    def _tester(actual: Tracer, asserts: typing.List):
        """
        :param actual: Tracer instance produced by executor fixture above
        :param asserts: List of tuples of the type
                    (
                        assertiontype <one of value, valuearray, classname, artifact>,
                        varname,
                        expectedvalue
                    )
        """
